...
class Admin_usuario extends formulario_basico {
    function asignar() {
       $_GET['id']=desencriptar_id($_GET['id']);
        $sql = "SELECT u.persona_id,
                    ...
                    p.segundo_apellido, ' [',p.identificacion,']') as persona_nombre
                FROM admin_usuario u, konecta.personas p
    ...
    function getSQL() {
        ...
        if ($_GET["nombre"] != "" && $_GET["nombre"] != "NULL") {
            ...
            $sql_extra = " WHERE CONCAT_WS(' ', p.nombre1, p.nombre2, p.apellido1) LIKE '%$t%' ";
        } 
        ...
        $sql = "SELECT
                    ...
                    p.id,
                    p.identificacion,
                    CONCAT_WS(' ', p.nombre1, p.nombre2, p.apellido1) AS nombre,
                    r.nombre as rol
                FROM admin_usuario u, admin_rol r,  konecta.personas p
                WHERE 
                        u.rol = r.id AND
                        u.persona_id=p.id
                        $s
