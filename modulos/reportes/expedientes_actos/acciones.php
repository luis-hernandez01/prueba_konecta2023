...
class Formulario extends Base {
    ...
    function mostrar() {
        ...
        $sql = "SELECT ex.*,ex.id as id_expediente, act.id as id_acto, ss.homologacion, ss.nombre as sector, sti.nombre as tipo_instrumento, sti.sigla, act.* ,YEAR(fecha_fin) as ano_fin, MONTH(fecha_fin) as mes_fin
        FROM expedientes_seguimiento  ex,
        sub_tipo_instrumentos sti,
        sub_sectores ss,
        acto_administrativo act
        WHERE act.expedientes_seguimiento_id=ex.id 
        and ss.id=ex.sub_sectores_id 
        and ( act.fecha_fin>='$fecha_inicio' and act.fecha_fin<='$fecha_fin' )
        and sti.id=ex.sub_tipo_instrumentos_id and act.visible=1  ORDER BY ano_fin,mes_fin ASC";
        ...
        <table style="font-family:'Times New Roman', Times, serif; font-size:8.5pt; 
            ...
            <thead>
                ...
                <tr style="font-weight:bold; text-align:left; background-color:<?= FONDO_HTML_TITULO ?>">
                    ...
                    <th style="width: 100pt">NÚMERO ACTO</th>
                    <th style="width: 100pt">FECHA INICIO</th>
                    <th style="width: 100pt">FECHA FIN</th>                    
                    <th style="width: 100pt">TIPO DE ACTO</th>
                    <th style="width: 100pt">AÑO ACTO</th>
                    <th style="width: 100pt">MES ACTO</th>
                </tr>
            </thead>
            <tbody>
                <?php
                while ($rw = $db->fetch_assoc($rs)) {
                    $fondo = ($fondo == "#fff" ? FONDO_HTML : "#fff");
                     $tipos_clasificacion = $this->db->select_one("SELECT nombre FROM tipo_acto_seguimiento WHERE id='$rw[tipo_acto_seguimiento_id]' ");
                    ?>
                    <tr style="text-align:left; background-color:<?= $fondo ?>" >
                        ...
                        <td style="width: 100pt"> <?php echo $rw["numero_acto"] ?></td>
        ...
        $nombre_archivo = "reporte_expedientes_actos";
