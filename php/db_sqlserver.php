...
class SQLSRV_DataBase {
	...
	public function make_update( $table, $what, $where = array() ) {
		$set   = '';
		$check = '';
	...
	public function select_row( $query, $format = 'array' ) {
		$request = $this->query( $query );
	...
	public function count_rows($sql){
		$data = $this->get_results($sql,'array');
		return count($data);
	}

	public function select_limit($sql,$numrows,$offset=0)
	{
		$sql.=" OFFSET $offset ROWS  FETCH FIRST $numrows ROWS ONLY";
		return $sql;
	}
	...
	public function select_all( $query, $format = 'array' ) {
		$response = array();
	...
	{
		return sqlsrv_fetch_array($result,SQLSRV_SCROLL_NEXT);
	}
	
	public function fetch_array ($result, $result_type = SQLSRV_FETCH_BOTH  ) 
	{
		return sqlsrv_fetch_array ($result, $result_type);
